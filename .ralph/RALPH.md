...
You are working through ONE slice of the parent PRD in `tjegbejimba/kindleflow`. The full issue body for the slice you must implement is appended at the end of this prompt under `--- ISSUE #N ---`.
...
- Use the `rubber-duck` agent (with both `gpt-5.5` and `claude-opus-4.7` in parallel) for plan critique instead of asking the user.
...
4. Read `UBIQUITOUS_LANGUAGE.md` if present and the parent PRD (`gh issue view 4 --repo tjegbejimba/kindleflow`) for vocabulary. Use those exact terms.
5. Check whether an open PR already references this issue:
   `gh pr list --repo tjegbejimba/kindleflow --state open --search "Closes #<N>"`.
...
5. **Local checks (all must pass)**:
   - Project checks: `Run the relevant checks documented by this repo.`
...
9. **Dual-model code review** (mandatory before merge):
   - Dispatch `code-review` agents in parallel against the open PR with both `gpt-5.5` and `claude-opus-4.7` (use `mode: "background"`, run them concurrently, then dedupe and merge findings).
...
11. **Fast merge respecting repo protections**:
    - First try the normal merge path: `gh pr merge <pr> --repo tjegbejimba/kindleflow --squash --delete-branch`.
    ...
    - If GitHub blocks the merge because required checks are pending, wait only for required checks: `gh pr checks <pr> --repo tjegbejimba/kindleflow --required --watch --fail-fast`, then retry the same normal merge command.
...
12. **Verify**: `gh issue view <N> --repo tjegbejimba/kindleflow --json state,closedByPullRequests` should show `state: CLOSED` and at least one entry in `closedByPullRequests` with `mergedAt != null`. If not, exit non-zero.
...
- The CI workflow must run on the Slice 1 PR itself. After merge, verify a successful run exists on `main` for the merge commit (`gh run list --repo tjegbejimba/kindleflow --branch main --limit 1`).
